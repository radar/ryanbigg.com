...
The guide sets up a +deploy+ user and a system-level RVM (Ruby Version Manager) install which contains Ruby 1.9.2. This user's home directory (+/home/deploy+) will include the application which we're wishing to deploy and this folder will be set up to be owned by the +deploy+ user so to restrict what this user has access to. The user does not have to be called +deploy+, it is used only as an example throughout this guide.

This guide is a basic instruction manual for getting a Rails application running on an Ubuntu 10.10 server. It covers the necessities of getting a server running, but it is not the "Everything You Ever Need To Deploy A Rails Application" guide.

Remember, KISS (the acronym, not the band)
...
WARNING: When setting up the server it's advised that you do not select "home directory encryption", as this will require a vital part of the installation, the private key used for log in, to be kept in an area outside of the common location of the home directory. It's a feature generally not needed for production servers as they are usually held in a secure datacenter.
...
If we don't have one of these we can generate a new one using the +ssh-keygen -t rsa+ command. We should enter a passphrase for this key which adds another layer of security on top, requiring a password for the first use of this key on a Linux or Mac machine. If the key is copied to another machine, then the password would be required to use the key there too. If we don't password-protect this key, anybody can copy the key to their machine and then use it to log into the server.
...
We should not be prompted to enter a password, if we are then something's gone Terribly, Terribly Wrong (TTW). Go back over the instructions here to see if a spot's been missed. When we log in as this user we'll take the security precaution of making this new +~/.ssh/authorized_keys+ file accessible only by this user by running this command:

<pre>
chmod 0600 ~/.ssh/authorized_keys
</pre>

The +chmod+ command ensures that this file is protected from people who are not our users writing to it and (potentially) adding their own key.

We should take care to run this on both user accounts and then test if we can log in still before continuing.
...
NOTE: There is also +/etc/ssh/ssh_config+, which is a little confusing... two files with nearly identical names. The file we're editing now is for the SSH *server* (or daemon, hence the +\d+), while the +ssh_config+ file is for the client. We should be making sure we're editing the right one.

We need to reload this configuration for the changes to take effect and the easiest way to do that would be to restart SSH, which can be done with this command:
...
Now we could install Ruby by downloading the package manually, un-gzip'ing it and un-tar'ing it, but that's boring. We could also install it using the package manager that comes with Ubuntu, but the Ruby that it provides is old and has been known to be broken.

Wouldn't it be nice if there was a tool that would do it for us? There is! It's called +rvm+!
...
With these packages installed, we'll have the minimum hassle possible when we go to install a Ruby version.
...
Next, we'll need to install the +bundler+ gem which will be used to set up the gems, such as Rails, for our application when we deploy it:
...
So now we've got a pretty good environment set up for our application, but we don't have our application on the server yet. To do this we need to undertake a process referred to as "deployment". It's with this process that we'll put our application's code on the server and be one step closer to letting people use the application.

Programmers don't like doing things that they can automate, manually. So one guy called Jamis Buck came up with a gem to assist with deployment called Capistrano.

h3. Capistrano

Capistrano is a gem originally created by Jamis Buck but is now maintained by Lee Hambley and additional volunteers, as well as the growing community that use it. It is designed for easy application deployment with the initial goal of being usable for Rails applications but can now be used for other applications.

In this section, we'll be using the "Ticketee":http://github.com/rails3book/ticketee application from <a href='http://manning.com/katz'>Rails 3 in Action</a> as an example. The "application is freely available":https://github.com/rails3book/ticketee on GitHub.

The majority of this section is also covered in the "official Capistrano guide":https://github.com/capistrano/capistrano/wiki/2.x-From-The-Beginning, which may provide some additional insight. This guide provides a concise walkthrough of every command that we'll use to set up a server.
...
The +capistrano+ gem must be installed on all computers which will be used to deploy the application, but _not_ the servers where the applications are deployed to. To make this happen rather than using the +gem install+ command we'll put this inside the +development+ group of our application's +Gemfile+:
...
To install this gem, we (and other developers of our application) are able to run +bundle install+. In this case, they'll be up-to-date with all gems rather than +gem install capistrano+ which would only update them with Capistrano.
...
The +shared/log+ directory is "symbolically linked":http://en.wikipedia.org/wiki/Symbolic_link to the current release's +log+ directory when we run a deploy. This is so that all logs will be kept in the +shared/log+ directory, rather than in each release.
...
* executing "chmod -R g+w /home/deploy/apps/ticketee/releases/20110206070736"
 servers: ["192.168.137.130"]
 [192.168.137.130] executing command
 command finished
</pre>
...
<pre>
* executing "rm -rf /home/deploy/apps/ticketee/releases/20110206070736/log /home/deploy/apps/ticketee/releases/20110206070736/public/system /home/deploy/apps/ticketee/releases/20110206070736/tmp/pids &&\\\n      mkdir -p /home/deploy/apps/ticketee/releases/20110206070736/public &&\\\n      mkdir -p /home/deploy/apps/ticketee/releases/20110206070736/tmp &&\\\n      ln -s /home/deploy/apps/ticketee/shared/log /home/deploy/apps/ticketee/releases/20110206070736/log &&\\\n      ln -s /home/deploy/apps/ticketee/shared/system /home/deploy/apps/ticketee/releases/20110206070736/public/system &&\\\n      ln -s /home/deploy/apps/ticketee/shared/pids /home/deploy/apps/ticketee/releases/20110206070736/tmp/pids"
   servers: ["192.168.137.130"]
   [192.168.137.130] executing command
   command finished
</pre>
...
<pre>
* executing "find /home/deploy/apps/ticketee/releases/20110206070736/public/images /home/deploy/apps/ticketee/releases/20110206070736/public/stylesheets /home/deploy/apps/ticketee/releases/20110206070736/public/javascripts -exec touch -t 201102060337.13 {} ';'; true"
   servers: ["192.168.137.130"]
   [192.168.137.130] executing command
   command finished
</pre>

The final step for the +deploy:update+ task is to run the +deploy:symlink+ task which symbolically links the new release directory to the +current+ folder within our deploy path (in this example, +/home/deploy/apps/ticketee/current+).

<pre>
* executing `deploy:symlink'
* executing "rm -f /home/deploy/apps/ticketee/current && ln -s /home/deploy/apps/ticketee/releases/20110206070736 /home/deploy/apps/ticketee/current"
   servers: ["192.168.137.130"]
   [192.168.137.130] executing command
   command finished
</pre>
...
We can trigger the +bundle install+ task to happen when we do a deploy by requiring the +bundler/capistrano+ file in the +config/deploy.rb+ of our application, right at the very top:
...
* executing "ls -x /home/deploy/apps/ticketee/releases"
  ...
  [192.168.137.130] executing command
  command finished
* executing "bundle install --gemfile /home/deploy/apps/ticketee/releases/20110206070736/Gemfile --path /home/deploy/apps/ticketee/shared/bundle --deployment --quiet --without development test"
  servers: ["192.168.137.130"]
  [192.168.137.130] executing command
  command finished
</pre>
...
!images/apache-stuff-not-found.png(Apache needs these packages)!
...
!images/apache2-module-instructions.png(Apache module instructions)!
...
We'll create a new file in this directory by using the +nano /etc/apache2/mods-available/passenger.load+ command (TODO: screenshot for nano?). Inside this file we'll put the configuration for loading the module. To enable this configuration we need to use the +a2enmod+ command like this:
...
!images/apache-virtual-host-instructions.png(Apache virtual host instructions)!

These instructions at the bottom of the output here tell us what we need to put into our Apache configuration to get this to run our application. Apache comes with a default file containing a +VirtualHost+ block, which just displays the welcome page we saw earlier. We're going to overwrite this file with our application's +VirtualHost+ configuration. We'll run +sudo nano /etc/apache2/sites-enabled/000-default+ (TODO: screenshot with nano) and then delete every line in this file, replacing it with the following:
...
TIP: A good idea would be to add a +ServerAlias+ underneath the +ServerName+ in this block, so that we can have other aliases to our server such as +www.our-server.com+. To do this, we'd write +ServerAlias www.our-server.com+. Other aliases go directly after, separated with a space: +ServerAlias www.our-server.com web.our-server.com+.

TIP: For Passenger configuration options, consult "the Passenger documentation":http://www.modrails.com/documentation/Users%20guide%20Apache.html.
...
NOTE: We're installing nginx this way as it will get us the latest (and therefore most stable) nginx, whereas the one provided by Ubuntu's packaging system may be out of date. This alternative method will also set up the configuration, which we'll see in a minute.

What we care about is when it stops, it will output this:

[Screenshot nginx passenger http instructions]

It even tells us what to edit ("probably +/opt/nginx/conf/nginx.conf+"). Genius! If we open this file now with +nano /opt/nginx/conf/nginx.conf+ (TODO: screenshot with nano) we'll see at the top of the +http+ block... oh! What it tells us to put there is already there. That's some smart thinking!
...
These scripts often take the +start+, +stop+ and +restart+ arguments, but can take others as well. When our server reboots, nginx, unfortunately, will not. To make this happen we need to create one of these init scripts and tell Ubuntu to run it on start up. Or, we could download one that was already created for us by using this command:

<pre>
sudo wget http://ryanbigg.com/deployment/scripts/nginx-init-script -O /etc/init.d/nginx
</pre>

This command will download the nginx init script and place it at +/etc/init.d/nginx+. Now to get this file to run when our system starts, we'll run these commands:
...
The first command changes the permission of the file to be executable, allowing it to be run. The second command runs +update-rc.d+ which is responsible for managing the init scripts for Ubuntu. This final line sets up nginx to be run during normal operation of our server.

If we were to reboot our server now, nginx would start automatically along with it. We don't need to do this right now, but it's good to know that it'll do it.

With all of this nginx setup we'll now be able to view our application. Let's go to http://192.168.137.130 and we'll see this.

[Screenshot for application homepage]

h4. Passenger configuration

Show example of RailsEnv and other useful options.